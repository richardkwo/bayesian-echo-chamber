...
def sample_bec(bec, display=20, sample_number=500, burn_in_number=500, resultDirName="./"):
    ...
    # sample
    for i in xrange(0, sample_number+burn_in_number):
    ...
    # time decay
    timeDecayHeader = "".join(["time.decay.%d\t" %(i) for i in xrange(1, A+1)])
    savetxt(outputfilename_time_decay, time_decays, header=timeDecayHeader, comments='')
    # word concentrations
    wordCocentrationHeader = "".join(["word.concentration.%d\t" %(i) for i in xrange(1, A+1)])
    savetxt(outputfilename_word_concentration, word_concentrations, comments='', header=wordCocentrationHeader)
    # influences
    influenceHeaders = "%s\t" * (A*A) % tuple(["influence.%d.%d\t" % (i,j) for i in xrange(1,A+1) for j in xrange(1,A+1)])
    savetxt(outputfilename_influence, 
            influences.reshape((shape(influences)[0], A*A)), header=influenceHeaders, comments='')
    # word pseudocounts
    wordPseudocountsHeader = "".join(["word.pseudocount.%d.for.word.%d\t" %(i,j) for i in xrange(1, A+1) for j in xrange(1,V+1)])
