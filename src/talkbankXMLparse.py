...
class talkbankXML(object):
    ...
    def __init__(self, xml_filename, remove_stop_words=True, min_number_msg=10, merge_consecutive_from_same_sender=True):
        '''
    ...
    def exportMessages(self, V, maxTime=100.0, display=0):
        ...
        print "\nTotal token counts:", sum([msg.get_total_token_counts() for x in xrange(self._A) for msg in allMessages[x]])
        print "Total token counts in raw text without restricting to top-V", w_count
        print "Removal rate = %.2f%%" % ((1-sum([msg.get_total_token_counts() for x in xrange(self._A) for msg in allMessages[x]]) * 1.0 / w_count)*100)
    ...
    def exportToLDAC(self, allMessages, time_to_split, fileNamePrefix="./"):
        ...
        for j in xrange(number_time_slices-1):
        ...
        print >>fw2, "%d" % (len(number_docs_times))
        ...
        for msg in messages:
            print >>fw4, "%d %.3f %d" % (msg.get_sender(), msg.get_start_time(), sum(msg.count_token_types()))
        fw4.close()
...
if __name__=="__main__":
    ...
    xml_filename = "../data/12-angry-men/12-angry-men.xml"
